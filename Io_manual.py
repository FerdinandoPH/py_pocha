class Io:
    ...
    def obtener_carta_a_jugar(self,jugador,vuelta):
        ...
        while carta_a_jugar < 0 or carta_a_jugar >= len(cartas_jugables):
            carta_a_jugar = int(input(f"Por favor, introduce un número entre 1 y {len(cartas_jugables)}: ")) - 1
    ...
    def mostrar_stats(self, jugadores):
        ...
        for jugador in jugadores_ordenados:
            print(f"{jugador.nombre}: {jugador.registro["puntos"]} ({jugador.registro["historial_variacion"][-1]})")
    def mostrar_vuelta(self, vuelta):
        print(vuelta)
        print(f"Ahora {vuelta.ganador.nombre} tiene {len(vuelta.ganador.vueltas)} bazas")
